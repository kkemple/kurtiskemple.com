...
description: "All system change occurs through exactly three operations: MOVE, JOIN, SEPARATE. Learn the universal law governing how boundaries transform and why the same mechanics that build can destroy."
...
Understanding COB transforms system change from art to engineering. The same operations that reduce entropy in the internet can increase it in a company. The same mechanics that enable breakthrough innovation can cause catastrophic failure. The direction depends entirely on how these operations are applied.

---

## The Universal Formula

System entropy change follows a precise mathematical relationship that accounts for both the operations performed and the resistance encountered. This formula applies whether optimizing a team of five or transforming a civilization of millions.
...
Where each variable captures a fundamental aspect of system change:
...
Historical MOVE operations that transformed civilization:
...
Transformative JOIN operations throughout history:
...
Revolutionary SEPARATE operations in history:
...
Conservation of Boundaries suggests several profound truths about how system entropy changes. These insights transform how we approach change at every scale.
...
Second, position matters more than plan. The most elegant strategy will fail if executed from a position of high entropy. Conversely, simple operations from low-entropy positions can change entire systems.
