...
featured: true
homepage:
  section: "developer-enablement-platform-strategy"
  order: 1
---
...
- **Public LLM directives.** An `llms.txt` file at the root of your domain. This is not a discovery mechanism—sitemaps already handle that. Its value is as a curated context artifact that users copy into a prompt or point an agent at directly. It should contain a closed logical chain of the most important things an agent could know about your platform and how to accomplish the most meaningful tasks with it, without needing any additional information. [Slack's llms.txt](https://docs.slack.dev/llms.txt), which I authored, follows this approach—a structured development guide that takes an agent from environment setup through building a production-ready Slack app to Marketplace submission, with every step, code example, and checkpoint included in a single document.
