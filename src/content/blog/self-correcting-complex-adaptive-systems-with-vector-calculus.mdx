...
When I [introduced context pollution](/blog/measuring-context-pollution) as a diagnostic signal for agentic systems, I focused on one specific problem: measuring how conversations wander from their original purpose. The math was simple—compare vector embeddings using cosine similarity.
...
You've seen it happen. An agentic assistant starts with a clear intent—"help me debug this code"—but twenty messages later it's explaining the history of computer science. This is intent drift in action.

As conversations progress, the context window fills with tangential information. Each response slightly shifts the focus. The agent adapts to recent messages rather than the original intent. Without a mechanism to detect this drift, conversations become increasingly incoherent until they're completely off-track.
...
- [Leaky Prompts: How Context Pollution Degrades AI Performance](/blog/leaky-prompts) - The original exploration of drift in agentic systems
- [Measuring Context Pollution in Agentic Systems](/blog/measuring-context-pollution) - Practical implementation for conversational agents
