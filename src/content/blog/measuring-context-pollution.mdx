...
title: "Measuring Context Pollution in AI Workflows"
description: "Context pollution measures the semantic distance between an agent's original task and its current direction. When that distance grows unchecked, the agent drifts outside its authority—optimizing for the wrong objective while appearing to function normally."
image: "/images/og/context-pollution.png"
pubDate: "07/19/2025"
featured: true
---

[Leaky prompts create context pollution](/blog/leaky-prompts) that degrades agent performance over time. But degradation is not the deepest problem. The deepest problem is that drift is invisible. An agent can move steadily away from its original task—outside the authority it was given—while producing outputs that look reasonable at every step. Without measurement, there is no signal. Without signal, there is no correction.

Context pollution is that signal. It captures the semantic distance between the agent's original task and where the conversation has actually gone—making drift visible, quantifiable, and actionable.
...
These thresholds turn drift from an invisible failure mode into a decision point—the system either stays within bounds or triggers intervention.

---

## Deep Research Workflow Example

Drift detection becomes concrete when built into the workflow itself. Consider an AI research assistant helping with a complex analysis project:
...
Re-anchoring does double duty. It realigns the agent with its original authority—the task it was given—and it manages context window limits by pruning outdated conversation history. The agent stays within bounds semantically and technically. Sessions continue indefinitely without degrading from drift or truncation.
...
These are not manual interventions. They are architectural decisions that create self-correcting workflows—the same generate-validate-correct pattern described in [Agentic Self-Correction](/blog/agentic-self-correction), applied to context alignment rather than output structure.

---

## How AI Can Help Close the Gap

As LLMs improve, they will not just suffer from drift—they will help detect it. An agent that can compute its own context pollution can detect when a user has unintentionally changed direction, suggest re-anchoring, and signal drift before it becomes a failure.

This is the [responsibility model](/blog/agentic-experience-design) in action. The agent operates within its responsibility—planning and outcomes—by monitoring its own alignment. The human retains judgment. The agent surfaces the signal that makes judgment possible. Drift detection does not give the agent more authority. It gives the human better visibility into whether the agent is still operating within the authority it was given.

---

## Why It Matters Now

As agents become embedded in multi-step workflows and long conversations, maintaining task coherence becomes critical. A 2% misalignment early in the chain can create a 40% failure rate by the end.
...
Leaky prompts describe the failure mode. Context pollution describes the technical cause. Context pollution measurement provides the signal that makes intervention possible.

Measurement without action is monitoring. Measurement with action—re-anchoring, context pruning, threshold-triggered intervention—is the beginning of self-correction. The agent detects its own drift, surfaces the signal, and either realigns or escalates. The [information architecture](/blog/llm-experience) defines the environment. The measurement layer tells you when that environment is degrading. The [correction loop](/blog/agentic-self-correction) brings it back.
